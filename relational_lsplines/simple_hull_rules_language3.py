...
class HullGeometryGenerator(object):
    ...
    def tree_search(self,
        ...
        enter the quagmire of circular reference quagmire:
            python weakref
            python 
...
def make_hull_rules_net(hullrulesnet):
    ...
    i.e. the waterline is no longer the design waterline
    #"""
    #Cld = Cld == lwl/draft
    ...
    #new try:
    #lfwl = lfwl == lfsac*ia(1.2,1.5)   #Oct 2018 standard
    lfwl = lfwl == lfsac*ia(1.8,2.5)   # oct 17 attempt more dwl flat
    hullrulesnet.set_rules(lfwl)
    ...
    #lfcp = lfcp == lfsac*ia(1.,1.1) #oct 2018 standard
    lfcp = lfcp == lfsac*ia(1.,1.05) #oct 17 look for flat mod
    ...
    #
    #Cp = Cp == ia(.6,.68)
    #Cp = Cp == ia(.68,.8) #june 20 #standard Oct.
    #Cp = Cp == ia(.8,.88) #July 06
    Cp = Cp == ia(.7,.88) #July 06/Oct 17 hi flat?
    ...
    #Acp = Acp >= draft*( lfcp + (lwl-lfcp)*ia(0.6,0.6) ) #no bbow
    Acp = Acp >= draft*( lfcp + (lwl-lfcp)*ia(0.9,0.9) ) 
...
def set_design_space():
    ...
    return DesignSpecification(
                            ...
                            bwl = (28.,34),#35.), #full width of vessel
                            #end Oct 14 2018
                            ...
                            #Oct14 2018:
                            #Clcg = (.45,.48), #location Coeff for the LCG
                            #moving LCG forward for CPK change.
                            #end Oct 14 2018
                            #
                            #
                            #Cb = (0.5,0.75), #fail safe ?
                            #Cb = (.5,.7),
                            #Cb = (.5,.75), #high prismatic, low block. (oct stand)
                            #Cb = (.65,.8), #Oct 17 more flat?
                            Cb = (.5,.75), #Oct 17 risky click.
                            #
                            #Cb = (0.5,0.7),
                            #Cb = (0.65,0.75),
                            #Cb = (0.55,0.8),
                            ...
                            #Cwp = (.89,.94),
                            ###Cwp = (.92,.96),
                            #Cwp = (.7,.8), #gar hull - to thin in the bow!
                            #Cwp = (.85,.9), #gar
                            Cwp = (.89,.98),
                            #Cwp = (.85,.9),
                            #
                            ...
                            #OCT 14 2018 change for CPK bbow
                            #Ccp = (.78,.87)
                            Ccp = (.84,.88)
                            )
"""Cwp should be > Ccp
...
#                                               11,12,13]  
        ...
        #HyperParameters.bulb_volume_fraction = .185
        #HyperParameters.bulb_length_fraction = .43
        #
        # good blend, to fat
        HyperParameters.bulb_volume_fraction = .18#.2#5
        HyperParameters.bulb_length_fraction = .35
        #
    ...
    ax=SD.hull.SAC.plotcurve_detailed(normalize=True,
                                      color_='black')
    xmin_tot, xmax_tot,ymin_tot, ymax_tot = SD.hull.SAC.extreme_C0()
    #xmax = ymax = 1.
    ax.fill_between(SD.hull.SAC.r[:,0]/xmax_tot,
                     0.,
                     SD.hull.SAC.r[:,1]/ymax_tot,
                     facecolor = 'green', 
                     alpha=.1,
                     label='area')
    ##
    ...
    xmin, xmax,ymin, ymax = SD.hull.Bulb_SAC.extreme_C0()
    ax.fill_between(SD.hull.Bulb_SAC.r[:,0]/xmax_tot,
                     0.,
                     SD.hull.Bulb_SAC.r[:,1]/ymax_tot,
                     facecolor = 'green', 
                     alpha=.1,
                     label='area')
    ##
    ...
    ax=SD.hull.DWL2D.plotcurve_detailed(normalize=True,
                                        color_='red',
                                        scale_curve=SD.hull.SAC,
                                        #scale_curve=SD.hull.DWL2D,
                                        #canvas=ax
                                        )
    xmin, xmax,ymin, ymax = SD.hull.DWL2D.extreme_C0()
    #xmin_tot, xmax_tot,ymin_tot, ymax_tot  = SD.hull.DWL2D.extreme_C0()
    ax.fill_between(SD.hull.DWL2D.r[:,0]/xmax_tot,
                     0.,
                     SD.hull.DWL2D.r[:,1]/ymax_tot,
                     facecolor = 'green', 
                     alpha=.1,
                     label='area')
    ##
    ...
    ax=SD.hull.CProfile2D.plotcurve_detailed(normalize=True,
                                             color_='blue',
                                             scale_curve=SD.hull.SAC,
                                             #scale_curve=SD.hull.CProfile2D,
                                             #canvas=ax
                                             )
    xmin, xmax,ymin, ymax = SD.hull.CProfile2D.extreme_C0()
    #xmin_tot, xmax_tot,ymin_tot, ymax_tot = SD.hull.CProfile2D.extreme_C0()
    ax.fill_between(SD.hull.CProfile2D.r[:,0]/xmax_tot,
                     0.,
                     SD.hull.CProfile2D.r[:,1]/ymax_tot,
                     facecolor = 'green', 
                     alpha=.1,
                     label='area')
