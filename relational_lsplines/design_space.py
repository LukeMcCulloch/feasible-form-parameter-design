...
class HullSpace(lp.States):
    SMALL = 1.e-3
    def __init__(self, strd_coeff=True):
        self.nstates = 0
        self._verbose = False
        self.valid_designs = True
        super(HullSpace, self).__init__( states = [self._setup_()])
    ...
    #no need to coverload all now
    #    def __call__(self, x): 
    #        d = []
    #        for st in self.states:
    #            d.append(st(x))
    #        return d
    # it was overloaded in States
    ...
    def make_lists(self):
        ...
        self.BowCurveCoeff = [self.bbfc,
                              self.dbfc,
                              self.Abfc,
                              self.Cbfc,]
        
        self.SternCurveCoeff = [self.bsfc,
                                self.dsfc,
                                self.Asfc,
                                self.Csfc,]
...
#                    self.arc_sets[c] = self.arc_sets[c].union(set(var.arcs))
    ...
    def AC_revise(self, print_=False, maxit = 100):
    ...
    def set_updates(self, states, states_old, vars, dlist=None):
        ...
        if dlist is None:
            for state, statei in zip(states, states_old):
                ...
                #chg_lst = [var for var in vars if (not state(var) == statei(var) and not (var in dlist))]             
...
#                    self.var_sets[c] = var_set
    ...
    def SAC_run_area_ini(self):
        ...
        dum = self(vol)
        if isinstance(dum[0], ia):
            vmax = max([el.sup for el in dum])
            dv = ia(0.,vmax)
            states = (self <= (SAC_mid_area,vmax))
        else:
    ...
    def SAC_run_area_consistency(self, niter=3):
        ...
        states = ( (self + (SAC_run_area,SAC_mid_area,c1) ) + (SAC_entrance_area,c1,vol))
        for i in range(niter):
    ...
    def constrain_SAC_Xc(self, niter=4):
        ...
        for i in range(niter):
            ...
            #"""
            states = (states + (.5, SAC_mid_len, b1) )
    ...
    def bow_fairness_section_area(self, niter=2):
        ...
        states = (self * (bbfc,dbfc,c1) / (Abfc,c1,Cbfc))
        for i in range(niter):
            states = (states <= (bbfc,bwl) )
            states = (states <= (dbfc,draft) )
    ...
    def shape_stern_section_inizers(self):
        ...
        vars   = [bsfc,dsfc,bwl,draft,Asfc,Csfc]
    ...
    def stern_fairness_section_area(self, niter=2):
        ...
        states = (self * (bsfc,dsfc,c1) / (Asfc,c1,Csfc) )
        for i in range(niter):
            states = (states <= (Csfc, ia(1.,1.)))
            states = (states <= (bsfc, bwl))
            states = (states <= (dsfc, draft))
...
if __name__ == "__main__":
    ...
    #self.Amsh = ia(-1.,1.)
    #
    #self = (self == (ia(2000.,300000.),self.vol) )
    self.draft = ia(5.,20.)
    self.dsmax = ia(5.,30.)
    
    
    self.SAC_entrance_area = ia(0.,100000.)
    self.SAC_mid_area = ia(0.,100000.)
    self.SAC_run_area = ia(0.,100000.)
    
    
    self.SAC_entrance_len = ia(0.,200.)
    self.SAC_mid_len = ia(0.,200.)
    self.SAC_run_len = ia(0.,200.)
    ...
    h2.vol =ia(Vsmall,Vbig)
    h2.lwl = ia(120.,Len)
    h2.lfwl = ia(0.,Len)
    h2.lfsac = ia(0.,Len)
    h2.lfos = ia(0.,Len)
    h2.bwl = ia(15.,BLEN) 
    h2.draft = ia(DLEN-h1.SMALL,DLEN)
    #"""