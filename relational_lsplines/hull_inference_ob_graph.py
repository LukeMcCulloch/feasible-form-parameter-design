...
class Hull(object):
    ...
    def AC_revise(self, print_=False, maxit = 10):
        ...
        self.var_set = set([])
        count=0
        while len(self.arc_set)>0 and count<maxit and self.valid_design:
    ...
    def equal(self, c1,c2):
        ...
        #print 'setting attribute equal to'
...
#        self.equal(self.Afos,Afos)
    ...
    def compute_LCG_coefficient(self, **args):
        if args:
            for arg in args:
                key = getattr(self, arg) 
                self.state = lp.Goal.eq(self.__dict__['_'+str(key)] , args[arg])(self.state)[0]
        self.state = self.CLCG_constraints()
        return
    ...
    def var_from_string(self, key):
        if isintance(key, str):
            return self.__dict__['_'+str(sac_section)]
        if isinstance(key, lp.Variable):
            return self.__dict__['_'+str(sac_section.name)]
    ...
    def constrain_SAC_Xc(self):
        ...
        g = lp.Goal.eq(c2,sct) #this one does not get used! - also
        # I haven't used this code in nearly a year.  (it is DEC 26,2017)
        # just an observation as I add CLCG to make
        # for hopefully more likely usually sensible LCGs!
    ...
    def CLCG_constraints(self):
        """NEVER tested here in 3 part SAC ~land~
        """
        lwl     = self.lwl
        LCG     = self.LCG
        Clcg     = self.Clcg
        ...
        vars = [lwl,LCG,Clcg]#, lfsac]
        statei = copy.copy(s)
        #g = lp.Goal.mullo(Clcg,lwl,LCG)
        g = lp.Goal.mulo(Clcg,lwl,LCG)
        state = g(s)[0]
...
#        del(state.values[c5])
    ...
    def SAC_run_area_ini(self):
        ...
        g = lp.Goal.lto(SAC_run_area,SAC_mid_area)
        state = g(s)[0]
        g = lp.Goal.lto(SAC_entrance_area,SAC_mid_area)
        state = g(s)[0]
    ...
    def SAC_entrance_len_consistency(self):
        ...
        #
        g = lp.Goal.lto(SAC_entrance_len,lwl)
        s = g(s)[0]
        g = lp.Goal.lto(SAC_run_len,lwl)
        s = g(s)[0]
        #
        #*********
        #
    ...
    def shape_bow_section_inizers(self):
        ...
        goal = lp.Goal.eq(Cbfc,ia(0.1,1.))
        s = goal(s)[0]
        
        bwlmax = self.state(bwl).sup
        goal = lp.Goal.eq(bbfc,ia(0.1,bwlmax))
        s = goal(s)[0]
        
        draftmax = self.state(draft).sup
        goal = lp.Goal.eq(dbfc,ia(.1,draftmax))
        state = goal(s)[0]
    ...
    def shape_stern_section_inizers(self):
        ...
        goal = lp.Goal.eq(Csfc,ia(0.1,1.))
        s = goal(s)[0]
        bwlmax = self.state(bwl).sup
        goal = lp.Goal.eq(bsfc,ia(0.1,bwlmax))
        s = goal(s)[0]
        draftmax = self.state(draft).sup
        goal = lp.Goal.eq(dsfc,ia(0.1,draftmax))
        state = goal(s)[0]
    ...
    def print_state(self, var=None):
        if var is None:
            ...
            #print '{}={}'.format(self.Afos, self.state(self.Afos) )
            print '\n bow fairness curve:'
